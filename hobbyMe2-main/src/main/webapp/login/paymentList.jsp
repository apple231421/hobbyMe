          ...
          <tbody>
            <c:forEach var="payment" items="${paymentList}" varStatus="status">
                <tr>
                    ...
                    <td class="text-start">
                        <a href="${pageContext.request.contextPath}/login/showPaymentConfim.do?post_num=${payment.myBoardNum}&id=${payment.myId}&state=${payment.myPayState}" class="text-decoration-none">
		                  ${payment.myPayTitle}
		                </a> 
                    </td>
                    <td>${payment.myId}</td>
        ...
        <ul class="pagination pagination-sm mb-0">
          <c:forEach var="i" begin="1" end="<%=total_page%>">
            <li class="page-item ${pageNum == i ? 'active' : ''}">
              <a class="page-link" href="<c:url value='/login/showPayment.do?pageNum=${i}' />">${i}</a>
            </li>
          </c:forEach>
        </ul>
      </nav>

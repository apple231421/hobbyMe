...
<div class="container py-4">
  ...
  <div class="bg-light p-4 rounded mb-4 shadow-sm">
    <h2 class="mb-1 fw-bold">내 보유 쿠폰 목록</h2>
    <p class="text-muted">사용자의 쿠폰을 확인하세요.</p>
  </div>

  <!-- 검색 및 전체 건수 -->
  <form action="<c:url value='/login/myCouponList.do'/>" method="post" class="mb-3">
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-3">
      ...
      <div class="d-flex gap-2">
        <select name="items" class="form-select form-select-sm" style="width: 120px;">
          <option value="coupon_content">제목</option>
        ...
        <ul class="pagination pagination-sm mb-0">
          <c:forEach var="i" begin="1" end="<%=total_page%>">
            <li class="page-item ${pageNum == i ? 'active' : ''}">
              <a class="page-link" href="<c:url value='/login/myCouponList.do?pageNum=${i}' />">${i}</a>
            </li>
          </c:forEach>
        </ul>
      </nav>
